...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .scale-block { display: flex; border: 1px solid var(--line); border-top: none; }
        .scale-col { flex: 1; padding: 6px 12px; border-right: 1px solid var(--line); font-size: 9.5px; }
        .scale-col:last-child { border-right: none; }
        .scale-title { font-weight: 700; font-size: 10px; margin-bottom: 3px; }
        .scale-row { display: flex; gap: 6px; margin-bottom: 1px; }
        .scale-score { font-weight: 700; min-width: 38px; }
        ...
        .matrix-table .section-row td { background: var(--section-bg); font-weight: 700; text-align: left; font-size: 10px; padding: 4px 8px; }
        .col-measure { width: 16%; } .col-opdef { width: 21%; } .col-quality { width: 16%; } .col-eff { width: 16%; } .col-time { width: 16%; } .col-source { width: 11%; } .col-del { width: 28px; }
        ...
        .view-scale-btn:hover { color: #c00; }
...
<div class="page">
    ...
    <form method="POST" action="{{ route('spcr.matrix.store') }}" id="mainForm">
        ...
        <div class="sig-outer">
            <div class="sig-row">
                <div class="sig-cell" style="flex:2;">
                    ...
                    <div class="date-row"><strong>Date:</strong> <input type="date" name="prepared_date" class="date-inp" value="{{ old('prepared_date') }}"></div>
                ...
                <div class="sig-cell" style="flex:2;">
                    ...
                    <div class="date-row"><strong>Date:</strong> <input type="date" name="reviewed_date" class="date-inp" value="{{ old('reviewed_date') }}"></div>
                ...
                <div class="sig-cell" style="flex:2;">
                    ...
                    <div class="date-row"><strong>Date:</strong> <input type="date" name="approved_date" class="date-inp" value="{{ old('approved_date') }}"></div>
        ...
        <div class="matrix-title-bar">Rating Matrix</div>
        <table class="matrix-table" id="matrixTable">
            ...
            <tbody id="matrixBody">
                <tr class="section-row">
                    <td colspan="6">
                        STRATEGIC FUNCTION
                        <button type="button" class="view-scale-btn" onclick="openScaleModal()">view</button>
                    </td>
...
document.getElementById('addSectionBtn').addEventListener('click', () => {
    ...
    inp.style.cssText = 'width:auto; border:none; background:transparent; font-weight:700; font-size:10px; outline:none;';
    const flag = document.createElement('input'); flag.type = 'hidden';
    flag.name = 'items[' + rowIdx + '][is_section]'; flag.value = '1'; rowIdx++;
    const viewBtn = document.createElement('button');
    viewBtn.type = 'button'; viewBtn.className = 'view-scale-btn';
    viewBtn.textContent = 'view'; viewBtn.onclick = openScaleModal;
    td.appendChild(inp); td.appendChild(flag); td.appendChild(viewBtn);
    const tdDel = document.createElement('td');
